	...
	spawnFlash() {
		return new Bluebird(async (resolve, reject) => {
			...
			if ("jetson-agx-orin-devkit" == this.deviceType.toString()) {
				const deltaDir = path.resolve(__dirname, '../delta');
				console.log("delta dir: " + deltaDir);
				walk(deltaDir, function (err, results) {
					if (err) throw err;
					for (const file of results) {
						if (path.basename(file).startsWith(".")) {
							continue;
						}
						const relPath = path.relative(deltaDir, file);
						const dir = path.dirname(relPath);
						fs.mkdirSync(join(workPath, dir), { recursive: true });
						console.log("Copying " + relPath + " to " + join(workPath, relPath));
						fs.copyFileSync(file, join(workPath, relPath));
					}
				});

				// sleep to wait out the promise above, i don't know how to javascript
				await new Promise(r => setTimeout(r, 3000));

				// use the balena-provided uefi image
				console.log("Copying balena-provided uefi_jetson.bin");
